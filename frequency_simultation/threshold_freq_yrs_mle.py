...
def poisson_likelihood(params, data):
    lambda_ = params[0]
    log_likelihood = np.sum(data * np.log(lambda_) - lambda_ - np.log(factorial(data)))
...
for i, df in enumerate(dataframes):
    ...
    # Generate some example observed data
    observed_data = df['counts'].values
    # Estimate the parameter using maximum likelihood estimation
    initial_guess = [46.67]  # Initial guess for the parameter
    result = minimize(poisson_likelihood, initial_guess, args=(np.array(df['counts'].tolist()),), method='Nelder-Mead')
    mu = result.x[0]
    # print("Estimated Lambda:", mu)
